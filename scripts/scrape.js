...
async function scrape(url, outputDir) {
    ...
    const hasAuth = fs_sync.existsSync(AUTH_FILE);

    if (!hasAuth) {
        console.error('No auth.json found. Run: node scripts/login.js first to login to Instagram.');
    }
    ...
    if (hasAuth) {
        ...
        console.error('Using saved Instagram session from auth.json');
    }
